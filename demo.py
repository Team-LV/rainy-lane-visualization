...
if __name__ == "__main__":
    ...
    input_video_path = "./input_video.mp4"  # 입력 영상 파일 경로
    output_video_path = "./output_video.mp4"  # 출력 영상 파일 경로

    cap = cv2.VideoCapture(input_video_path)
    fourcc = cv2.VideoWriter_fourcc(*'mp4v')
    out = cv2.VideoWriter(output_video_path, fourcc, 25, (320, 240))
    ...
    while True:
        ...
        # frame = frame[151:240, 1:320] # 이 줄 들어오면 영상 재생 안됨

        frame = MSR(frame, variance_list)
        # frame, minv = wrapping(frame) # 이 줄 들어오면 도로가 아닌 하늘 영상이 나옴
        # frame = cv2.cvtColor(frame, cv2.COLOR_BGR2GRAY) # 이 줄 들어오면 영상 재생 안됨
        value, frame = threshold(frame) # 이 줄 들어오면 화면 밝기 매우 낮아짐, 차선 검출도 더 안되는 듯?
